      ...
      <form method="post" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
        ...
        <div class="lg:w-1/2 md:w-2/3 mx-auto">
          <div class="flex flex-wrap -m-2">
            ...
            <div class="p-2 w-full">
              <div class="relative">
                <label for="is_new" class="block leading-7 text-sm text-gray-600">新着商品</label>
            ...
            <div class="p-2 w-full">
              <div class="relative">
                <label for="is_sold_out" class="block leading-7 text-sm text-gray-600">売り切れ表示 (新着商品と一緒にチェックを入れない）</label>
                <input type="hidden" name="is_sold_out" value="0">
                <input type="checkbox" name="is_sold_out" value="1" {{ old('is_sold_out') ? 'checked' : '' }} class="mr-2">
                <label class="text-sm">売り切れ表示にする</label>
              </div>
            </div>
            <div class="p-2 w-full">
              <div class="relative">
                <label for="is_active" class="block leading-7 text-sm text-gray-600">公開状態</label>
                <select name="is_active" class="border rounded px-3 py-2">
                  <option value="1" {{ old('is_active', 1) == 1 ? 'selected' : '' }}>公開</option>
                  <option value="0" {{ old('is_active', 1) == 0 ? 'selected' : '' }}>非公開</option>
                </select>
              </div>    
            </div>
            <div class="p-2 w-full">
